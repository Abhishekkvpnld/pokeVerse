    ...
    return (
        <div className="min-h-screen gap-3 bg-gray-100 flex flex-col items-center justify-center">
            ...
            <div className="w-full px-10 mt-2 flex items-center justify-between">
                ...
                <Link href={"/pokemon"} className="flex gap-2 border px-2 py-1 rounded-2xl hover:scale-110 transition hover:underline hover:bg-amber-700 hover:text-white items-center"><FaArrowLeft /> Back</Link>
            </div>
            ...
            <div className="bg-white h-full p-6 rounded-xl shadow-lg w-[90%] text-center flex flex-col  md:flex-row items-center">
                    ...
                    <div className="mt-3 flex items-center justify-center gap-2">
                        <div className="text-gray-700 flex items-center gap-2">
                            ...
                            {pokemon?.types?.map((data, index) => (
                                <div key={index} className="bg-red-200 px-3 py-1 rounded-lg text-sm">⚡ {data?.type?.name}</div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="flex-1">
                    <div className="mt-3 flex flex-col items-start">
                        <div className="flex items-center mb-1">  <h3 className="font-semibold">Abilities:</h3></div>
                        <div className="text-gray-700 flex items-center gap-2">
                            {pokemon?.abilities?.map((data, index) => (
                                <div key={index} className="bg-green-200 px-3 py-1 rounded-lg text-sm">⚡ {data?.ability?.name}</div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="mt-4">
                        ...
                        <div className="w-full rounded-2xl shadow-sm">
                            ...
                            {pokemon?.stats?.map((statObj, index) => (
                                <div key={index} className=" flex mt-2 bg-slate-300 rounded-2xl hover:bg-blue-500 hover:shadow-2xl items-center justify-between px-4">
                                    <p className="px-3 py-2 font-medium text-gray-700 hover:text-white">{statObj.stat.name}</p>
                                    <p className="px-3 py-2 rounded-full bg-white text-gray-600">{statObj.base_stat}</p>
                                </div>
                            ))}
                    ...
                    <div className="mt-4">
                        ...
                        <ul className="text-gray-700 flex flex-wrap justify-center gap-2">
                            {pokemon?.moves?.slice(0, 10).map((data, index) => (
                                <li
                                    key={index}
                                    className="bg-yellow-200 px-3 py-1 mt-1 rounded-lg text-sm"
                                >
                                    {data?.move?.name}
                                </li>
                            ))}
